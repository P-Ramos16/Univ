...
def main():
    ...
    tab = sorted(tabela, key=lambda x: 3*x[V]+x[E], reverse=True)
    printTabela(tab)

    # c)
    # Acrescente os argumentos adequados à função sorted para
    # obter uma tabela ordenada por ordem decrescente da diferença GM-GS:
    tab = sorted(tabela, key=lambda y: y[GM]-y[GS], reverse=True)
    printTabela(tab)
    ...
    tab = sorted(tabela, key=lambda z: (3*z[V]+z[E], z[GM]-z[GS]), reverse=True)
    printTabela(tab)
