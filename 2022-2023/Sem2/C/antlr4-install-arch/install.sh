...
echo ""
echo "This scripts accepts a previously installed java jdk (eg: sudo pacman -S openjdk-11-jdk)."
...
echo ""
echo "Phase #3: installing jar files and update zshrc (sudo required, and zsh shell usage assumed)"
...
if [[ $ans != "n" && $ans != "N" ]]; then
   ...
   echo "- updating CLASSPATH and add ANTLR4_PATH to ~/.zshrc file:"
   # commenting existing variables (if any):
   sed -i 's/\(^\<export\> \<CLASSPATH\>.*$\)/\#\1/g' ~/.zshrc || (echo "ERROR: changing ~/.zshrc file!"; exit 3)
   sed -i 's/\(^\<export\> \<ANTLR4_PATH\>.*$\)/\#\1/g' ~/.zshrc || (echo "ERROR: changing ~/.zshrc file!"; exit 3)
   # add new:
   echo "export CLASSPATH=\".:"$JAR_DST_PATH"/${ANTLR4File}:"$JAR_DST_PATH"/${STFile}\"" >> ~/.zshrc || (echo "ERROR: changing ~/.zshrc file!"; exit 3)
   echo "export ANTLR4_PATH=\""$JAR_DST_PATH"\"" >> ~/.zshrc || (echo "ERROR: changing ~/.zshrc file!"; exit 3)
   echo "Phase #3 done."
fi
...
if [[ $ans != "n" && $ans != "N" ]]; then
   ...
   if [[ $ans != "n" && $ans != "N" ]]; then
      ...
      echo "    sudo pacman -S openjdk-doc"
   fi
   echo "Phase #5 done."
fi
...
echo ""
echo "Start a new terminal before start ANTLR4ring! (necessary to ensure that the new ./zshrc is executed)"
