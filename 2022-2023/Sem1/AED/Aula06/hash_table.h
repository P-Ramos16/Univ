...
hash_table_node* find_hash_table_node(char key[64])
{
  ...
  if(hash_table[index] != NULL) {
    hash_table_node* lastKey = hash_table[index];
    hash_table_node* lastGoodKey = hash_table[index];

    while (lastKey->next != NULL) {
      if (strcmp(lastKey->key, key)) {
        lastGoodKey = lastKey;
      }
...
void insert_hash_table_node(hash_table_node *n)
{
  ...
  hash_table_node* lastNode = find_hash_table_node(index);

  if (lastNode != NULL) {
    while (lastNode->next != NULL) {
      if (strcmp(lastNode->key, n->key)) {
        valInc = lastNode->value;
      }
      lastNode = lastNode->next;
    }
    hash_table_node* newNode = allocate_hash_table_node();
    strcpy(*newNode->key, *n->key);
    newNode->value = valInc;
